    ...
    if bb.data.inherits_class("autotools", d):
        ...
        if "intltool-native" in depends:
            if not check_configure("[AI][CT]_PROG_INTLTOOL"):
                ...
                oe.qa.handle_error("configure-depends", error_msg, d)
    ...
    try:
        ...
        if bool(statinfo.st_mode & stat.S_IXUSR):
            if subprocess.call("grep -q -F 'libtool wrapper (GNU libtool)' %s" % path, shell=True) == 0:
                oe.qa.handle_error("libtool-wrapper", "%s: %s looks like a libtool wrapper script" % (name, package_qa_clean_path(path, d, name)), d)
...
def package_qa_check_many_debug(pn, d, messages):
    ...
    for pkg in d.getVar("PACKAGES").split():
        if pkg.endswith("-dbg"):
            if seen:
                oe.qa.handle_error("many-debug", "%s: has more than one -dbg package" % pn, d)
                return False
            seen = True
    return True
...
def package_qa_check_src_uri(pn, d, messages):
    ...
    if "${DEBIAN_MIRROR}" in srcuri:
        oe.qa.handle_error("src-uri-bad", "%s: SRC_URI uses DEBIAN_MIRROR not archive.debian.org" % pn, d)
    if "${PN}" in srcuri:
        oe.qa.handle_error("src-uri-bad", "%s: SRC_URI uses PN not BPN" % pn, d)
    if re.search(r"github\.com/.+/.+/archive/.*", srcuri):
        oe.qa.handle_error("src-uri-bad", "%s: SRC_URI uses unstable github archives" % pn, d)
...
def package_qa_missing_pn(pkg, d, messages):
    if d.getVar("PN") == pkg:
        ...
        if len(contents) == 0 and d.getVar("ALLOW_EMPTY_" + pkg) != "1":
            oe.qa.handle_error("missing-pn", "%s: primary package not generated" % pkg, d)
...
def package_qa_check_bsd_license(pn, d, messages):
    ...
    # TODO: also LICENSES_*
    if "BSD" in licenses:
        oe.qa.handle_error("bsd-license", "%s: uses deprecated BSD license" % pn, d)
